...
function requireProfile(req, res, next) {
    ...
    if (!profileId) {
        return res.status(400).json({ error: 'El parámetro "profile" es requerido.' });
    }
    next();
}

// Función para obtener parámetros comunes de la solicitud
async function getProfileParams(req) {
    const profileId = req.query.profile || req.params.profileId;
...
async function getFromApi(apiUrl, username, password, action, additionalParams = '') {
    try {
        const response = await axios.get(`${apiUrl}/player_api.php?username=${username}&password=${password}&action=${action}${additionalParams}`);
...
function authenticateResetToken(req, res, next) {
    ...
    if (!token) {
        return res.status(401).json({ error: 'Token requerido para restablecer la contraseña.' });
    }
...
app.post('/register', validateApiKey, async (req, res) => {
    ...
    try {
        ...
        bcrypt.hash(password, saltRounds, async (err, hashedPassword) => {
            ...
            const verificationCode = await generateUniqueVerificationCode(); // Generar un código único
            ...
            try {
                await sendEmail(email, { firstName, lastName, verificationCode }, 'verify'); // Enviar el código de verificación de cuenta
                res.status(201).json({ message: 'Usuario registrado exitosamente. Por favor, verifica tu correo para activar tu cuenta.' });
            } catch (emailError) {
                await User.deleteOne({ email });
                res.status(500).json({ error: 'Error al enviar el correo de verificación. Tu cuenta no ha sido creada.' });
            }
        });
    } catch (error) {
...
app.post('/verify', validateApiKey, async (req, res) => {
    ...
    try {
        ...
        await sendEmail(user.email, { firstName: user.firstName, lastName: user.lastName, username: user.username }, 'congratulations');
        ...
        res.status(500).json({ message: 'Hubo un error al verificar la cuenta. Inténtalo de nuevo.' });
    }
});
...
app.post('/resend-verification', validateApiKey, async (req, res) => {
    ...
    try {
        ...
        const newVerificationCode = await generateUniqueVerificationCode(); // Generar un nuevo código de 6 dígitos
        ...
        console.error('Error en el reenvío del correo de verificación:', error.message);
        res.status(500).json({ error: 'Error al procesar la solicitud.' });
    }
});

// Inicio de sesión
app.post('/login', validateApiKey, 
    ...
    async (req, res) => {
        ...
        try {
            ...
            bcrypt.compare(password, user.password, (err, result) => {
                ...
                if (!user.isVerified) {
                    return res.status(400).json({ error: 'Cuenta no verificada. Por favor, verifica tu cuenta antes de iniciar sesión.' });
                }
...
app.post('/forgot-password',validateApiKey, async (req, res) => {
    ...
    try {
        ...
        const verificationCode = await generateUniqueVerificationCode(); // Generar un nuevo código de 6 dígitos
        ...
        console.error('Error en el proceso de recuperación de contraseña:', error.message);
        res.status(500).json({ error: 'Error al procesar la solicitud.' });
    }
});
...
app.post('/verify-reset-code', validateApiKey, async (req, res) => {
    ...
    try {
        ...
        // Si no se encuentra el usuario, devolver un error
        if (!user) return res.status(404).json({ error: 'Código de verificación inválido.' });
        ...
        console.error('Error al verificar el código:', error.message);
        res.status(500).json({ error: 'Hubo un error al verificar el código. Inténtalo de nuevo.' });
    }
});

// Endpoint para restablecer la contraseña después de verificar el código
app.post('/reset-password', validateApiKey, authenticateResetToken, async (req, res) => {
    ...
    try {
        ...
        if (!user.verificationToken) {
            return res.status(400).json({ message: 'Tu contraseña ya ha sido cambiada anteriormente.' });
        }
        ...
        res.status(500).json({ error: 'Hubo un error al restablecer la contraseña. Inténtalo de nuevo.' });
    }
});
...
app.post('/recover-username', validateApiKey, async (req, res) => {
    ...
    try {
        ...
        if (!user) {
            return res.status(404).json({ error: 'No se encontró una cuenta asociada a ese correo.' });
        }

        // Enviar el correo con el nombre de usuario
        await sendEmail(email, { firstName: user.firstName, lastName: user.lastName, username: user.username }, 'username-recovery');

        res.status(200).json({ message: 'El nombre de usuario ha sido enviado a tu correo.' });

    } catch (error) {
        console.error('Error al enviar el correo de recuperación de nombre de usuario:', error.message);
        res.status(500).json({ error: 'Hubo un error al procesar tu solicitud. Inténtalo de nuevo.' });
    }
});
...
app.post('/profiles', validateApiKey, authenticateToken, 
    ...
    async (req, res) => {
        ...
        try {
            ...
            const existingProfile = await Profile.findOne({ userId: req.user.userId, username });
            if (existingProfile) {
                return res.status(400).json({ error: 'Ya existe un perfil con ese nombre de usuario para este usuario' });
            }
    
            // Crear un nuevo perfil
            const profile = new Profile({ userId: req.user.userId, url, username, password });
            await profile.save();
...
app.get('/profiles/:profileId', validateApiKey, authenticateToken, async (req, res) => {
...
app.get('/vods/recent', validateApiKey, authenticateToken, requireProfile, async (req, res) => {
    try {
        // Obtener el perfil asociado con la solicitud
        const profile = await getProfileParams(req);
...
app.get('/vods/categories', validateApiKey, authenticateToken, requireProfile, async (req, res) => {
    try {
        const profile = await getProfileParams(req);
        const { url, username, password } = profile;

        const categories = await getFromApi(url, username, password, 'get_vod_categories');

        res.json(categories);
    } catch (error) {
        console.error('Error al obtener las categorías de VODs:', error.message);
        res.status(500).json({ error: 'Error al obtener las categorías de VODs' });
    }
});

// Obtener todos los VODs paginados
app.get('/vods', validateApiKey, authenticateToken, requireProfile, async (req, res) => {
    try {
        const profile = await getProfileParams(req);
        const { url, username, password } = profile;
        const page = parseInt(req.query.page) || 1;
...
app.get('/vods/category/:categoryId', validateApiKey, authenticateToken, requireProfile, async (req, res) => {
    try {
        const profile = await getProfileParams(req);
        const { url, username, password } = profile;
        ...
        const vods = await getFromApi(url, username, password, 'get_vod_streams', `&category_id=${categoryId}`);
        ...
        res.status(500).json({ error: 'Error al obtener los VODs por categoría' });
    }
});

// Obtener detalles de un VOD específico
app.get('/vods/:vodId', validateApiKey, authenticateToken, requireProfile, async (req, res) => {
    try {
        const profile = await getProfileParams(req);
        const { url, username, password } = profile;
        const vodId = req.params.vodId;

        const vodDetail = await getFromApi(url, username, password, 'get_vod_streams', `&vod_id=${vodId}`);
...
app.get('/series/recent', validateApiKey, authenticateToken, requireProfile, async (req, res) => {
    try {
        const profile = await getProfileParams(req);
        const { url, username, password } = profile;
        const page = parseInt(req.query.page) || 1;
        ...
        const sortedSeries = limitedSeries.sort((a, b) => new Date(b.added) - new Date(a.added));
...
app.get('/series/categories', validateApiKey, authenticateToken, requireProfile, async (req, res) => {
    try {
        const profile = await getProfileParams(req);
        const { url, username, password } = profile;

        const categories = await getFromApi(url, username, password, 'get_series_categories');

        res.json(categories);
    } catch (error) {
        console.error('Error al obtener las categorías de series:', error.message);
        res.status(500).json({ error: 'Error al obtener las categorías de series' });
    }
});

// Obtener todas las series paginadas
app.get('/series', validateApiKey, authenticateToken, requireProfile, async (req, res) => {
    try {
        const profile = await getProfileParams(req);
        const { url, username, password } = profile;
        const page = parseInt(req.query.page) || 1;
...
app.get('/series/category/:categoryId', validateApiKey, authenticateToken, requireProfile, async (req, res) => {
    try {
        const profile = await getProfileParams(req);
        const { url, username, password } = profile;
        const categoryId = req.params.categoryId;
        ...
        const series = await getFromApi(url, username, password, 'get_series_streams', `&category_id=${categoryId}`);
        ...
        console.error('Error al obtener las series por categoría:', error.message);
        res.status(500).json({ error: 'Error al obtener las series por categoría' });
    }
});

// Obtener detalles de una serie específica
app.get('/series/:seriesId', validateApiKey, authenticateToken, requireProfile, async (req, res) => {
    try {
        const profile = await getProfileParams(req);
        const { url, username, password } = profile;
        const seriesId = req.params.seriesId;

        const seriesDetail = await getFromApi(url, username, password, 'get_series_streams', `&series_id=${seriesId}`);

        res.json(seriesDetail);
    } catch (error) {
        console.error('Error al obtener los detalles de la serie:', error.message);
        res.status(500).json({ error: 'Error al obtener los detalles de la serie' });
    }
});

// ----------------------------------- Live Streams Endpoints -----------------------------------

app.get('/live/recent', validateApiKey, authenticateToken, requireProfile, async (req, res) => {
    try {
        const profile = await getProfileParams(req);
        const { url, username, password } = profile;
        const page = parseInt(req.query.page) || 1;
        ...
        const liveStreams = await getFromApi(url, username, password, 'get_live_streams');
        ...
        const sortedLiveStreams = limitedLiveStreams.sort((a, b) => new Date(b.added) - new Date(a.added));

        // Paginación de los live streams
        const paginatedLiveStreams = sortedLiveStreams.slice((page - 1) * limit, page * limit);
        ...
        console.error(error);
        res.status(500).json({ error: 'Error al obtener los live streams recientes' });
    }
});

// Obtener todas las categorías de live streams
app.get('/live/categories', validateApiKey, authenticateToken, requireProfile, async (req, res) => {
    try {
        const profile = await getProfileParams(req);
        const { url, username, password } = profile;

        const categories = await getFromApi(url, username, password, 'get_live_categories');

        res.json(categories);
    } catch (error) {
        console.error('Error al obtener las categorías de live streams:', error.message);
        res.status(500).json({ error: 'Error al obtener las categorías de live streams' });
    }
});

// Obtener todos los live streams paginados
app.get('/live', validateApiKey, authenticateToken, requireProfile, async (req, res) => {
    try {
        const profile = await getProfileParams(req);
        const { url, username, password } = profile;
        const page = parseInt(req.query.page) || 1;
        ...
        const paginatedLiveStreams = liveStreams.slice((page - 1) * limit, page * limit);
...
app.get('/live/category/:categoryId', validateApiKey, authenticateToken, requireProfile, async (req, res) => {
    try {
        const profile = await getProfileParams(req);
        const { url, username, password } = profile;
        const categoryId = req.params.categoryId;
        ...
        const liveStreams = await getFromApi(url, username, password, 'get_live_streams', `&category_id=${categoryId}`);
        ...
        console.error('Error al obtener los live streams por categoría:', error.message);
        res.status(500).json({ error: 'Error al obtener los live streams por categoría' });
    }
});

// Obtener detalles de un live stream específico
app.get('/live/:liveId', validateApiKey, authenticateToken, requireProfile, async (req, res) => {
    try {
        const profile = await getProfileParams(req);
        const { url, username, password } = profile;
        const liveId = req.params.liveId;

        const liveStreamDetail = await getFromApi(url, username, password, 'get_live_streams', `&live_id=${liveId}`);

        res.json(liveStreamDetail);
    } catch (error) {
        console.error('Error al obtener los detalles del live stream:', error.message);
        res.status(500).json({ error: 'Error al obtener los detalles del live stream' });
    }
});
...
app.get('/search', validateApiKey, authenticateToken, requireProfile, async (req, res) => {
    try {
        const profile = await getProfileParams(req); // Obtener el perfil seleccionado
        const { url, username, password } = profile;
